...
class TestQuestionController extends Controller
{
    ...
    /**
     * Create a question inside a test section.
     */
    ...
    private function updateDetailRecord(TestQuestion $question, string $type, array $data): void
    {
        ...
        match (true) {
            ...
            $type === 'gap_fill' => (function () use ($qId, $data) {
                $detail = TestGapFillQuestion::where('test_question_id', $qId)->first();
                if ($detail) {
                    $detail->update(array_filter([
            ...
            $type === 'rephrase' => (function () use ($qId, $data) {
                $detail = TestRephraseQuestion::where('test_question_id', $qId)->first();
            ...
            $type === 'word_formation' => (function () use ($qId, $data) {
                $detail = TestWordFormationQuestion::where('test_question_id', $qId)->first();
                if ($detail) {
                    $detail->update(array_filter([
            ...
            $type === 'replace' => (function () use ($qId, $data) {
                $detail = TestReplaceQuestion::where('test_question_id', $qId)->first();
                if ($detail) {
                    $detail->update(array_filter([
            ...
            $type === 'correct' => (function () use ($qId, $data) {
                $detail = TestCorrectQuestion::where('test_question_id', $qId)->first();
                if ($detail) {
                    $detail->update(array_filter([
            ...
            $type === 'word_derivation' => (function () use ($qId, $data) {
                $detail = TestWordDerivationQuestion::where('test_question_id', $qId)->first();
                if ($detail) {
                    $detail->update(array_filter([
            ...
            $type === 'text_completion' => (function () use ($qId, $data) {
                $detail = TestTextCompletionQuestion::where('test_question_id', $qId)->first();
                if ($detail) {
                    $detail->update(array_filter([
            ...
            $type === 'correlation' => (function () use ($qId, $data) {
                $detail = TestCorrelationQuestion::where('test_question_id', $qId)->first();
                if ($detail) {
                    $detail->update(array_filter([
