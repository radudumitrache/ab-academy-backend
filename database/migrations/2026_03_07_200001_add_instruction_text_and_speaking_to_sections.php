...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!Schema::hasColumn('homework_sections', 'instruction_text')) {
            Schema::table('homework_sections', function (Blueprint $table) {
                $table->text('instruction_text')->nullable()->after('title');
            });
        }

        // MySQL requires re-specifying the full enum to add a value (SQLite has no enum enforcement)
        ...
        if (!Schema::hasColumn('test_sections', 'instruction_text')) {
